...
3.	Para el departamento de logística:
	...
	A.	Es necesario saber la cantidad camiones que harian falta para transportar los materiales que se utilizarán para la construcción
      de un edificio. Para ello, se ingresa la cantidad de toneladas necesarias de materiales a transportar. El programa deberá informar 
      la cantidad de camiones, sabiendo que cada uno de ellos puede transportar por viaje 3500kg
...
class App(customtkinter.CTk):
    ...
    def btn_cantidad_camiones_on_click(self):
        ...
        cantidad_de_camieones = math.ceil(tonelada / camion) 
        alert(title = "Cantidad de Camiones", message = f"La cantidad de camiones necesarios es: {cantidad_de_camieones} camiones "   )

    def btn_tiempo_llegada_on_click(self):
        kilometros = float (self.txt_kilometros.get())
        horas = math.ceil (kilometros / 90)
        alert(title ="Tiempo estimado",message = f"el horario estimado es de: {horas} hs")
